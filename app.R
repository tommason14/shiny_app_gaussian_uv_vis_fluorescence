...
ui <- fluidPage(
    titlePanel("Plot Gaussian UV-Vis and Fluorescence"),
    sidebarLayout(
        sidebarPanel(
...
# Define server logic required to extract data and create plot
server <- function(input, output) {
    
    # reactive component allows for both the displaying and saving
    # of the plot- otherwise ggsave tries to save the plot as well
    # as the HTML metadata.
    plotdata <- reactive({
      ...
      smooth_with_gaussians <- function(df, sigma, step){
        ...
        for(i in 1:length(df$Intensity)){ # want the index
            new_ints = new_ints + df$Intensity[i] * normpdf(new_energies, 
                                                            df$Transitions[i],
                                                            sigma)
        }
      ...
      plot_gaussians <- function(df){
        ...
        req(input$file$datapath) # prevent the message "'con' is not a connection" before user adds a file
        f <- readLines(input$file$datapath)
        ...
        data.frame(Transitions = transitions,
                   Wavelength = waves, 
                   Intensity = osc) %>% 
            tail(nstates) %>% 
            add_gaussians() %>% 
    ...
    output$download <- downloadHandler(
        ...
        content = function(file) {
            ggsave(file, plotdata(), dpi = 300, width=6, height=5)
        })
}
